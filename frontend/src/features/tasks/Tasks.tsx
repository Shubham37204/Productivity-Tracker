...
export default function Tasks() {
  ...
  useEffect(() => {
    if (!api) return
    api.get('api/tasks/').json<Task[]>().then(setTasks).catch(console.error)
  }, [api])
  ...
  const handleAdd = async () => {
    ...
    setLoading(true)
    try {
      const task = await api.post('api/tasks/', { json: { title, category, xp: 10 } }).json<Task>()
      addTask(task)
      setTitle('')
    } finally {
      setLoading(false)
  ...
  const handleToggle = async (task: Task) => {
    ...
    const updated = await api.put(`api/tasks/${task.id}`, { json: { completed: true } }).json<Task>()
    ...
    <div className="space-y-8">
      ...
      <Card className="shadow-sm border-border">
        <CardContent className="pt-6 pb-6 space-y-4">
          <div>
            <label className="text-sm font-semibold text-foreground mb-1.5 block">Task title</label>
            <Input
                ...
                <Card className="shadow-sm hover:shadow-md transition-shadow">
                  <CardContent className="py-4 px-5 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className={`inline-block mt-1 text-xs px-2 py-0.5 rounded-md font-medium ${CATEGORY_COLORS[task.category] || CATEGORY_COLORS.General}`}>
                        {task.category}
                      </span>
                    </div>
                    <div className="flex items-center gap-3 shrink-0 relative">
                      ...
                      <button onClick={() => handleDelete(task.id)}>
                        <Trash2 size={16} className="text-muted-foreground hover:text-red-500 transition-colors" />
                      </button>
                    </div>
                  </CardContent>
                </Card>
          ...
          {done.map((task) => (
            <Card key={task.id} className="opacity-50">
              <CardContent className="py-4 px-5 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-base line-through text-muted-foreground truncate">{task.title}</p>
                  <span className={`inline-block mt-1 text-xs px-2 py-0.5 rounded-md font-medium ${CATEGORY_COLORS[task.category] || CATEGORY_COLORS.General}`}>
                    {task.category}
                  </span>
                </div>
